...
/**
 * @title RealisticCrossChainTest
 * @notice Realistic cross-chain test with separate routers per chain
 */
contract RealisticCrossChainTest is TestBase {
    ...
    function _registerCrossChainStrategy() internal {
        ...
        bytes4[4] memory selectors = [
            baseStrategy.deposit.selector,
            baseStrategy.withdraw.selector,
            bytes4(0),
            bytes4(0)
        ];
        
        crossChainStrategyId = strategyManager.registerStrategy(
            "Base Strategy",
    ...
    function deliverMessage(
        ...
        for (uint256 i = 0; i < msgData.tokens.length; i++) {
            tokenAmounts[i] = Client.EVMTokenAmount({
                token: msgData.tokens[i],
                amount: msgData.amounts[i]
            });
        }
