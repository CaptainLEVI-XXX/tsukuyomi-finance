...
import {MockERC20} from "./Base.t.sol";
...
contract PoolManagerTest is TestBase {
    ...
    function test_DepositAndWithdraw() public {
        ...
        // Alice deposits USDC
        vm.startPrank(alice);
        MockERC20(USDC).approve(address(poolManager), depositAmount);
        
        uint256 sharesBefore = poolManager.balanceOf(alice, usdcTokenId);
        uint256 poolBalanceBefore = MockERC20(USDC).balanceOf(address(poolManager));
        ...
        uint256 poolBalanceAfter = MockERC20(USDC).balanceOf(address(poolManager));
        {
        ...
        // Check asset info
        (address asset, uint96 totalShares, uint128 totalAssets, uint128 allocatedToStrategy, string memory name, string memory symbol, uint8 decimals, bool isActive, uint32 lastUpdateTime, uint64 totalYieldEarned) = poolManager.assets(usdcTokenId);
    ...
    function test_MultipleDepositsAndShareValue() public {
        ...
        vm.startPrank(alice);
        MockERC20(USDC).approve(address(poolManager), aliceDeposit);
        ...
        MockERC20(USDC).approve(address(poolManager), yield);
        ...
        vm.startPrank(bob);
        MockERC20(USDC).approve(address(poolManager), bobDeposit);
    ...
    function test_StrategyAllocationAndReturn() public {
        ...
        console.log("test_StrategyAllocationAndReturn");
        ...
        // Assertions
        assertEq(allocatedToStrategy, allocationAmount, "Allocated amount incorrect");
        assertEq(poolBalanceBefore - poolBalanceAfter, allocationAmount, "Pool balance change incorrect");
        assertEq(MockERC20(USDC).balanceOf(address(strategyManager)), allocationAmount, "Strategy manager balance incorrect");
        
        // Return funds with yield
        uint256 yieldAmount = 150e6; // 10% yield
        _dealTokens(USDC, address(strategyManager), yieldAmount); // Give strategy manager yield
        
        MockERC20(USDC).approve(address(poolManager), allocationAmount + yieldAmount);
        console.log("allocationAmount", allocationAmount);
        console.log("yieldAmount", yieldAmount);
        console.log("allocationAmount + yieldAmount", allocationAmount + yieldAmount);

        poolManager.returnFromStrategy(usdcTokenId, allocationAmount, yieldAmount);
        
        ( asset,  totalShares,  totalAssets, allocatedToStrategy, name, symbol, decimals, isActive, lastUpdateTime, totalYieldEarned) = poolManager.assets(usdcTokenId);
    ...
    //     // Approve all tokens
    //     MockERC20(USDC).approve(address(poolManager), usdcAmount);
    //     MockERC20(USDT).approve(address(poolManager), usdtAmount);
    //     MockERC20(DAI).approve(address(poolManager), daiAmount);
    ...
    //     // Check TVL (all should be in USD)
    //     uint256 tvl = poolManager.getTotalUSDValue();
    //     uint256 expectedTVL = 1_000e8 + 2_000e8 + 500e8; // $3,500 with 8 decimals
    //     assertEq(tvl, expectedTVL, "TVL calculation incorrect");
