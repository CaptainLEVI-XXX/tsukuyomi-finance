...
export class PortfolioBuilder {
    ...
    async buildStrategies(pools, amount, marketConditions) {
    ...
    async buildStrategy(pools, amount, riskLevel, marketConditions) {
    ...
    filterPoolsByRisk(pools, profile) {
        return pools.filter(pool => {
            // Risk score filter
            if (pool.riskScore > profile.maxRiskScore)
                return false;
            // TVL filter
            if (pool.tvlUsd < profile.minTVL)
                return false;
            // Protocol age filter
            if (pool.protocolAge < profile.minProtocolAge)
                return false;
            // Pool type filter
            if (profile.allowedPoolTypes[0] !== 'all' &&
                !profile.allowedPoolTypes.includes(pool.poolType))
                return false;
            // APY sanity check
            if (pool.apy < 1 || pool.apy > 200)
                return false;
            return true;
        });
    }
    scoreAndRankPools(pools, riskLevel, marketConditions) {
    ...
    optimizeAllocations(rankedPools, totalAmount, profile, riskLevel) {
        const recommendations = [];
        let remainingAmount = totalAmount;
        // Diversification tracking
        const protocolExposure = {};
        const poolTypeExposure = {};
        ...
        for (const pool of rankedPools.slice(0, targetPositions * 2)) { // Consider 2x positions
            if (recommendations.length >= targetPositions)
                break;
            if (remainingAmount < totalAmount * 0.05)
                break; // Min 5% allocation
            ...
            if (currentProtocolExposure >= profile.maxAllocationPerProtocol)
                continue;
            if (currentTypeExposure >= 60)
                continue; // Max 60% in one type
            // Calculate optimal allocation
            const baseAllocation = 100 / targetPositions;
            const scoreMultiplier = (pool.score || 1) / (rankedPools[0].score || 1);
            let allocation = baseAllocation * (0.8 + scoreMultiplier * 0.4);
            // Apply constraints
            allocation = Math.min(allocation, profile.maxAllocationPerProtocol - currentProtocolExposure, (remainingAmount / totalAmount) * 100);
            ...
            const recommendation = this.createPoolRecommendation(pool, allocation, amountUSD, recommendations.length + 1);
    ...
    createPoolRecommendation(pool, allocation, amountUSD, poolId) {
    ...
    determineTokenAllocations(pool) {
    ...
    calculateStrategyMetrics(allocations, riskLevel) {
        const totalExpectedAPY = allocations.reduce((sum, alloc) => sum + (alloc.adjustedAPY * alloc.allocation / 100), 0);
        const totalRiskScore = allocations.reduce((sum, alloc) => sum + (alloc.pool.riskScore * alloc.allocation / 100), 0);
        const estimatedAnnualReturn = allocations.reduce((sum, alloc) => sum + alloc.expectedReturn, 0);
    ...
    normalizeAllocations(recommendations, totalAmount) {
