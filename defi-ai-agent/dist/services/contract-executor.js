...
export class ContractExecutor {
    ...
    async executeInvestment(poolId, recommendation) {
        ...
        try {
            ...
            // Estimate gas
            const gasEstimate = await this.contract.investCrossChain.estimateGas(poolId, recommendation.strategyId, tokenIds, percentages, targetAsset);
            ...
            const tx = await this.contract.investCrossChain(poolId, recommendation.strategyId, tokenIds, percentages, targetAsset, {
                gasLimit,
                gasPrice
            });
    ...
    async harvestYield(strategyId, assets) {
        try {
    ...
    async performPreExecutionChecks(recommendation) {
    ...
    selectOptimalAsset(recommendation) {
        const chainTokens = TOKEN_ADDRESSES[this.chain.toUpperCase()];
        if (!chainTokens) {
